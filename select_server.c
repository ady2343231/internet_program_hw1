...
int main(){
	...
	//if(setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, &flag, reuselen) == -1){
	//	printf("reuse err");
	//	return 0;
	//}
	...
	int	maxfd, fdi;
	fd_set	all_set, copy_fd;

	FD_ZERO(&copy_fd);
	FD_ZERO(&all_set);

	maxfd = listenfd;
	FD_SET(listenfd, &copy_fd);
